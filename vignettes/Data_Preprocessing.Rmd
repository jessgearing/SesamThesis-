...
We load the already classified behavioral data. There are single files per sensor. Here we combine the individual cow sensor data into one large dataset.  
```{r}
...
#now we can plot
violin_activity <- ggplot(df, aes(series, value, fill = series)) + geom_violin() +
  xlab("Behaviour") + ylab("Time spent per day (minutes)")
ggsave(violin_activity, file = "~/sesam/MSc_project/Thesis_docs/violin_activity.pdf")
violin_activity
```

Further outlier removal for behaviours 
```{r}
nrow(all_activity)
#lying
Q1 <- quantile(all_activity$Lying, .25)
Q3 <- quantile(all_activity$Lying, .75)
IQR <- IQR(all_activity$Lying)

all_activity <- subset(all_activity, all_activity$Lying > (Q1 - 3*IQR) & all_activity$Lying < (Q3 + 3*IQR))
nrow(all_activity)

#Eating
Q1 <- quantile(all_activity$Eating, .25)
Q3 <- quantile(all_activity$Eating, .75)
IQR <- IQR(all_activity$Eating)
all_activity <- subset(all_activity, all_activity$Eating > (Q1 - 3*IQR) & all_activity$Eating < (Q3 + 3*IQR))

#Standing
Q1 <- quantile(all_activity$Standing, .25)
Q3 <- quantile(all_activity$Standing, .75)
IQR <- IQR(all_activity$Standing)
all_activity <- subset(all_activity, all_activity$Standing > (Q1 - 3*IQR) & all_activity$Standing < (Q3 + 3*IQR))

#Walking
Q1 <- quantile(all_activity$Walking, .25)
Q3 <- quantile(all_activity$Walking, .75)
IQR <- IQR(all_activity$Walking)
all_activity <- subset(all_activity, all_activity$Walking > (Q1 - 3*IQR) & all_activity$Walking < (Q3 + 3*IQR))

#Ruminating
Q1 <- quantile(all_activity$Ruminating, .25)
Q3 <- quantile(all_activity$Ruminating, .75)
IQR <- IQR(all_activity$Ruminating)
all_activity <- subset(all_activity, all_activity$Ruminating > (Q1 - 3*IQR) & all_activity$Ruminating < (Q3 + 3*IQR))

all_activity2 <- all_activity

save(all_activity2,
     file="~/sesam/MSc_project/Thesis_docs/data/all_activity_SESAM_COW2.RData",
     vebose=T)  
...
### melted_SESAM_COW.RData is the file we require now for standardisation 
save(df2,
     file="~/sesam/MSc_project/Thesis_docs/data/melted_SESAM_COW.RData",
     vebose=T)


plot_mean2 <- ggplot(df2, aes(series, value, fill = series)) + geom_violin(scale="count", width=1.5) 
plot_mean2 + stat_summary(fun.data="mean_sdl", mult=1, 
